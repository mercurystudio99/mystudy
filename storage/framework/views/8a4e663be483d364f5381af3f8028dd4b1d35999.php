    ...
    <?php if(!empty($heroSectionData)): ?>
        ...
        <section class="slider-container  <?php echo e(($heroSection == "2") ? 'slider-hero-section2' : ''); ?>" <?php if(empty($heroSectionData['is_video_background'])): ?> style="background-image: url('<?php echo e($heroSectionData['hero_background']); ?>')" <?php endif; ?>>
            ...
            <div class="container user-select-none">
                ...
                <?php if($heroSection == "2"): ?>
                    <div class="row slider-content align-items-center hero-section2 flex-column-reverse flex-md-row">
                        <div class="col-12 col-md-7 col-lg-6">
                            ...
                            <form action="<?php echo e(route('search')); ?>" method="get" class="d-inline-flex mt-30 mt-lg-30 w-100">
                ...
                <?php else: ?>
                    <div class="text-center slider-content mb-2" style="height: 50vh;">
                        <h1><?php echo e($heroSectionData['title']); ?></h1>
                        <div class="row h-100 align-items-center justify-content-center text-center">
                            <div class="col-12 col-md-9 col-lg-7 mb-2">
                                <p class="mt-30 slide-hint"><?php echo nl2br($heroSectionData['description']); ?></p>

                                <form action="<?php echo e(route('search')); ?>" method="get" class="search_row d-inline-flex mt-30 mt-lg-50 p-20" style="margin-left: -8rem!important;width:145%!important">
                                <div class="form-group d-flex align-items-center m-0 slider-search p-20 bg-white w-145" style="margin-left: 0rem; flex-wrap: nowrap; display: flex; align-items: center;">
    <div class="dropdown" style="margin-right: auto; padding-left: 10px">
    <select id="levelSelect" class="form-control" name="combined_id">
        <option value="" selected disabled>All Levels</option>
        
        <?php $__currentLoopData = $levels; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $level): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <optgroup label="<?php echo e($level->title); ?>">
                <option value="level_<?php echo e($level->id); ?>" class="level_id"><?php echo e($level->title); ?></option>
                
                <?php
                    $sublevels = DB::table('sub_levels')->where('level_id', $level->id)->get();
                ?>
                    <?php if($sublevels): ?>
                <?php $__currentLoopData = $sublevels; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sublevel): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option value="level_<?php echo e($level->id); ?>_sublevel_<?php echo e($sublevel->id); ?>" class="ml-2 sublevel_id">&#9656; <?php echo e($sublevel->title); ?></option>
        ...
        <div class="dropdown" style="margin-right: auto; padding-left: 10px">
            <select id="getSubjects" class="form-control" name="subject_id">
                <option value="" selected disabled>All Subjects</option>
        ...
        <div class="dropdown" style="margin-right: auto; padding-left: 10px">
            <select id="countrySelect" class="form-control" name="country">
                ...
                <?php $__currentLoopData = $countries; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $countryTitle): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($countryTitle); ?>"><?php echo e($countryTitle); ?></option>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        ...
        <div class="dropdown" style="margin-right: auto; padding-left: 10px">
            <select id="getDistricts" class="form-control" name="district">
        ...
        <div class="dropdown" style="margin-right: auto; padding: 10px">
            <select id="getLocalities" class="form-control" name="locality">
                <option value="" selected disabled>All Localities</option>
...
<?php $__env->stopPush(); ?>
  ...
  <!-- Include jQuery in your HTML head -->
<script src="https://code.jquery.com/jquery-3.6.4.min.js" integrity="sha256-XXX" crossorigin="anonymous"></script>
...
<script>
    $(document).ready(function () {
        ...
        var combined_id = $(this).val();
            var matches = combined_id.match(/^level_(\d+)_sublevel_(\d+)$/);
            var sublevel_id = $(this).find('.sublevel_id').val();
            var single_level_id = $(this).find('.level_id').val();
            var single_level_id = single_level_id.replace(/^level_/i, "");


            if(matches){
                // var sublevel_id = sublevel_id.replace(/^level_\d+_sublevel_/i, "");
                var level_id = matches[1];
                var sublevel_id = matches[2];
                data = {
    level_id: level_id,
...
$.ajax({
    url: '/getSubjects',  // Replace with your actual API endpoint
    type: 'GET',
    data: data,
        dataType: 'json',
    success: function (response) {
        // Assuming the response has a 'subjects' property
        var subjects = response.subjects;
        if(subjects && subjects.length > 0){
            $('#getSubjects').html('');

$.each(subjects, function (index, subject) {
    $('#getSubjects').append('<option value="' + subject.id + '">' + subject.title + '</option>');
    // Adjust the HTML structure based on your subject object properties
});
        }
        
        // Update the content of the div with id 'getSubjects'
...
// Make an AJAX request
$.ajax({
    url: '/getDistricts',  // Replace with your actual API endpoint
    type: 'GET',
    data: data,
        dataType: 'json',
    success: function (response) {
        // Assuming the response has a 'subjects' property
        var districts = response.districts; // Assuming response is the array you posted
if (districts && districts.length > 0) {
    $('#getDistricts').html('');

    $.each(districts, function (index, district) {
        $('#getDistricts').append('<option value="' + district.district + '">' + district.district + '</option>');
        // Adjust the HTML structure based on your subject object properties
    });
}
...
// Make an AJAX request
$.ajax({
    url: '/getLocalities',  // Replace with your actual API endpoint
    type: 'GET',
    data: data,
        dataType: 'json',
    success: function (response) {
        // Assuming the response has a 'subjects' property
        var localities = response.localities; // Assuming response is the array you posted
if (localities && localities.length > 0) {
    $('#getLocalities').html('');

    $.each(localities, function (index, locality) {
        $('#getLocalities').append('<option value="' + locality.locality + '">' + locality.locality + '</option>');
        // Adjust the HTML structure based on your subject object properties
    });
}
...
<?php echo $__env->make(getTemplate().'.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/n1c7f25/public_html/resources/views/web/default/pages/home.blade.php ENDPATH**/ ?>