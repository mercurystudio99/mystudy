...
unset($__errorArgs, $__bag); ?>" id="course_type">
                ...
                <option value="session" <?php if(!empty($webinar) and $webinar->type == 'session'): ?> selected <?php endif; ?>><?php echo e(trans('webinars.session')); ?></option>
            </select>
...
unset($__errorArgs, $__bag); ?>
        ...
        <div class="form-group session_fields" id="session_fields">
            <label>Banner</label>
            <div class="input-group">
                <div class="input-group-prepend">
                    <button type="button" class="input-group-text panel-file-manager" data-input="banner_image" data-preview="holder">
                        <i data-feather="arrow-up" width="18" height="18" class="text-white"></i>
                    </button>
                </div>
                <input type="text" name="banner" id="banner" value="<?php echo e(!empty($webinar) ? $webinar->banner : old('banner')); ?>" placeholder="<?php echo e(trans('forms.course_thumbnail_size')); ?>" class="form-control <?php $__errorArgs = ['banner'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>  is-invalid <?php unset($message);
...
unset($__errorArgs, $__bag); ?>" aria-label="Default select example" id="level_id" name="level_id">
                            <option selected disabled>Select any option</option>
                            <?php $__currentLoopData = $levels; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $level): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($level->id); ?>" <?php echo e(@$level->id == @$webinar->level_id ? 'selected' : ''); ?>><?php echo e($level->title); ?></option>
...
unset($__errorArgs, $__bag); ?>" aria-label="Default select example" id="getSublevels" name="sublevel_id">
                            <option selected disabled>Select any option</option>
                        </select>
...
unset($__errorArgs, $__bag); ?>" aria-label="Default select example" name="subject_id" id="getSubjects">
                            <option selected disabled>Select any option</option>
                        </select>
...
unset($__errorArgs, $__bag); ?></div>
                    ...
                    <div class="form-group session_fields" id="session_fields">
                        <label class="form-select">Address</label><br>
                        <select class="form-control <?php $__errorArgs = ['address_id'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> is-invalid <?php unset($message);
...
unset($__errorArgs, $__bag); ?>" aria-label="Default select example" name="address_id">
                            <option selected disabled>Select any option</option>
                            <?php $__currentLoopData = $addresses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $address): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($address->id); ?>" <?php echo e(@$address->id == @$webinar->address_id ? 'selected' : ''); ?>><?php echo e($address->address1); ?></option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </select>
                        <div class="invalid-feedback"><?php $__errorArgs = ['address_id'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> <?php echo e($message); ?> <?php unset($message);
...
unset($__errorArgs, $__bag); ?></div>
                    ...
                    <div class="form-group session_fields">
                    <label class="input-label">Start Time (Minutes)</label>
                    <div class="input-group">
                        <div class="input-group-prepend">
                        ...
                        <input type="time" name="start_time" value="<?php echo e((!empty($webinar) and !empty($webinar->start_time)) ? $webinar->start_time : old('start_time')); ?>" class="form-control timePicker <?php $__errorArgs = ['start_time'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>  is-invalid <?php unset($message);
...
unset($__errorArgs, $__bag); ?>
                ...
                <div class="form-group session_fields">
                    <label class="input-label">End Time (Minutes)</label>
                    <div class="input-group">
                        <div class="input-group-prepend">
                            <span class="input-group-text" id="timeInputGroupPrepend">
                                <i data-feather="clock" width="18" height="18" class="text-white"></i>
                        ...
                        <input type="time" name="end_time" value="<?php echo e((!empty($webinar) and !empty($webinar->end_time)) ? $webinar->end_time : old('end_time')); ?>" class="form-control timePicker <?php $__errorArgs = ['end_time'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>  is-invalid <?php unset($message);
...
<script>
   $(document).ready(function () {
    // Add an event listener for the change event on the #course_type dropdown
    ...
    $('#course_type').trigger('change');
        ...
        $.ajax({
                    type: "POST",
                    url: "<?php echo e(route('webinars.getSublevels')); ?>", // Specify the route URL
                    data: {
                        level_id: level_id
                    },
    success: function (response) {
        // Assuming the response has a 'subjects' property
        var sublevels = response.sublevels;
        if(sublevels && sublevels.length > 0){
            $('#getSublevels').html('');

    $.each(sublevels, function (index, sublevel) {
    $('#getSublevels').append('<option value="' + sublevel.id + '">' + sublevel.title + '</option>');
    ...
    $("#level_id").change(function() {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo e(route('webinars.getSublevels')); ?>",
            data: {
                level_id: level_id
            },
            success: function (response) {
                ...
                if (sublevels && sublevels.length > 0) {
                    ...
                    // Trigger change on the sublevel dropdown
                    $('#getSublevels').trigger('change');
                }
            },
            error: function (xhr, status, error) {
                console.error('Error:', status, error);
    ...
    $("#getSublevels").change(function() {
        var sublevel_id = $(this).val();
        var level_id = $('#level_id').val();
        $.ajax({
            type: "POST",
            url: "<?php echo e(route('webinars.getSubjects')); ?>",
            data: {
...
</script><?php /**PATH /home/n1c7f25/public_html/resources/views/web/default/panel/webinar/create_includes/step_1.blade.php ENDPATH**/ ?>