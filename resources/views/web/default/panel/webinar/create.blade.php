...
@section('content')
    ...
    <div class="">
        ...
        <form method="post" action="/panel/webinars/{{ !empty($webinar) ? $webinar->id .'/update' : 'store' }}" id="webinarForm" class="webinar-form" enctype="multipart/form-data">
            ...
            <input type="hidden" id="old_session" value="{{@$webinar['type']}}">
            {{ csrf_field() }}
        ...
        <div class="create-webinar-footer d-flex flex-column flex-md-row align-items-center justify-content-between mt-20 pt-15 border-top">
            <div class="d-flex align-items-center">
                ...
                @if($webinar->type=='session')
                    <a href="/panel/webinars/{{ $webinar->id }}/step/{{($currentStep == 8) ? ($currentStep - 4) : (($currentStep == 4) ? ($currentStep - 3) : ($currentStep - 1)) }}" class="btn btn-sm btn-primary {{ $currentStep < 2 ? 'disabled' : '' }}">{{ trans('webinars.previous') }}</a>
                @else
