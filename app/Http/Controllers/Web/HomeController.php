...
class HomeController extends Controller
{
    ...
    public function search(Request $request){
        $data = $request->all();
        $combined_id = $request->input('combined_id');

        $pattern = '/^level_(\d+)_sublevel_(\d+)$/';
        $webinarsQuery = Webinar::where('type', 'session')->where('status','active');
    // Perform the regular expression match
    if (preg_match($pattern, $combined_id, $matches)) {
        // $matches[1] will contain the level ID
        $level_id = $matches[1];

        // $matches[2] will contain the sublevel ID
        $sublevel_id = $matches[2];
        if ($sublevel_id) {
            $webinarsQuery->where('sublevel_id', $sublevel_id);
        }
        
        // Check if level_id exists in the request
        if ($level_id) {
            $webinarsQuery->where('level_id', $level_id);
        }
    }
    

// Check if sublevel_id exists in the request


// Check if subject_id exists in the request
if ($request->has('subject_id')) {
    $webinarsQuery->where('subject_id', $request->subject_id);
}

// Check if location_id exists in the request
if ($request->has('country')) {
    $webinarsQuery->where('address_id', $request->location_id);
}
if ($request->has('locality')) {
    $location_id = Location::where('locality', $request->locality)->value('id');
} elseif($request->has('district')){
    $location_id = Location::where('district', $request->district)->value('id');  
} elseif($request->has('country')){
    $location_id = Location::where('country', $request->country)->value('id'); 
}
if ($request->has('locality')) {
    $webinarsQuery->where('address_id', $location_id);
}
...
foreach($webinars as $webinar){
    $subject_title = Subject::where('id',$webinar->subject_id)->value('title');
    $webinar['subject_title'] = $subject_title;
}
$resultCount = count($webinars);
return view('web.default.pages.search', compact('webinars','resultCount'));
    }
    public function getSubjects(Request $request){
        if($request->sublevel_id){
            $subjects = Subject::where('level_id',$request->level_id)->where('sublevel_id',$request->sublevel_id)->get();
        } else{
    ...
    public function getLocalities(Request $request){
        $localities = Location::select('locality')
