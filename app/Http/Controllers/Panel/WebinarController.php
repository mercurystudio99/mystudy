...

{
    ...
    public function getSublevels(Request $request){
        $sublevels = Sublevel::where('level_id',$request->level_id)->get();
        return response()->json(['message' => 'Record fetched successfully', 'sublevels' => $sublevels]);
     }

     public function getSubjects(Request $request){
        $subjects = Subject::where('level_id',$request->level_id)->where('sublevel_id',$request->sublevel_id)->get();
        return response()->json(['message' => 'Record fetched successfully', 'subjects' => $subjects]);
     }
...

    {
        ...
        if($data['type']=='session'){
            $rules = [
                ...
                'type' => 'required|in:webinar,course,text_lesson,session',
        ...
        if($request->type=='session'){
            ...
            if ($webinar) {
                ...
                $quiz->save();
                $faq = new Faq();
                $faq->creator_id = $user->id;
                $faq->webinar_id = $webinar->id;
                $faq->save();
                $chapter = new WebinarChapter();
            ...
            $webinar = Webinar::create([
                ...
                'slug' => Webinar::makeSlug($request->title),
...

    {
        ...
        if ($getNextStep) {
            ...
            if($webinar->type=='session'){

                $currentStep = ($currentStep == 1) ? 3 : (($currentStep == 4) ? 7 : $currentStep);
